/*! AmazeUI - v1.0.0 | (c) 2014 AllMobilize, Inc. | @license MIT | 2014-07-24 10:07:55 */ /*! Sea.js 2.2.1 | seajs.org/LICENSE.md */
...
define("list_news", [], function(require, exports, module) {
    var $ = window.Zepto, listNewsInit = function() {
        if ($(".am-list-news-one").length) {
            ...
            if ($(".am-list").children().length > 6) {
                $(".am-list").children().each(function(index) {
                    if (index > 5) {
                        $(this).hide();
                    }
                });
                $(".am-list-news-one").append($listMore);
            }
            $(".am-list-news-more").on("click", function() {
                $(".am-list").children().show();
                $(this).hide();
            });
        }
    };
...
define("map", [ "core", "zepto.extend.fx", "zepto.extend.data", "zepto.extend.selector" ], function(require, exports, module) {
    ...
    function addBdMap() {
        var $content = $(".am-map"), name = $content.attr("data-name"), address = $content.attr("data-address");
...
define("ui.modal", [ "core", "zepto.extend.fx", "zepto.extend.data", "zepto.extend.selector", "ui.dimmer" ], function(require, exports, module) {
    ...
    $doc.on("click", "[data-am-modal]", function() {
        var $this = $(this), options = UI.utils.parseOptions($this.attr("data-am-modal")), $target = options.target && $(options.target) || this.href && this.href.replace(/.*(?=#[^\s]+$)/, ""), option = $target.data("am.modal") ? "toggle" : options;
        Plugin.call($target, option, this);
    });
    module.exports = Modal;
});
define("ui.offcanvas", [ "./zepto.outerdemension", "./zepto.extend.data", "./zepto.extend.selector", "./core", "zepto.extend.fx", "zepto.extend.data", "zepto.extend.selector" ], function(require, exports, module) {
    require("./zepto.outerdemension");
    require("./zepto.extend.data");
    require("./zepto.extend.selector");
    require("./core");
    ...
    OffCanvas.prototype.open = function() {
        var _self = this, $target = $(this.options.target), openEvent = $.Event("open:offcanvas:amui");
        if (!$target.length) return;
        var effect = this.options.effect, $html = $("html"), $bar = $target.find(".am-offcanvas-bar").first(), dir = $bar.hasClass("am-offcanvas-bar-flip") ? -1 : 1, scrollBar = dir == -1 && $win.width() < window.innerWidth ? window.innerWidth - $win.width() : 0;
        ...
        if (!(effect === "overlay")) {
            $html.css("margin-left", ($bar.outerWidth() - scrollBar) * dir).width();
        }
        setTimeout(function() {
            $bar.addClass("am-offcanvas-bar-active").width();
        }, 25);
        $doc.trigger(openEvent);
        $target.off(".offcanvas.amui").on("click.offcanvas.amui swipeRight.offcanvas.amui swipeLeft.offcanvas.amui", function(e) {
            var $target = $(e.target);
    ...
    OffCanvas.prototype.close = function(force) {
        var $html = $("html"), $panel = $(".am-offcanvas.am-active"), $bar = $panel.find(".am-offcanvas-bar").first();
        $doc.trigger("close:offcanvas:amui");
        if (!$panel.length) return;
        if ($.AMUI.support.transition && !force) {
            $html.one($.AMUI.support.transition.end, function() {
        ...
        $panel.off(".offcanvas.amui");
    };
    OffCanvas.prototype.events = function() {
        var $element = this.$element;
        /*$element.off('.offcanvas.amui').on('click.offcanvas.amui',
            $.proxy(function(e) {
                e.preventDefault();
                this.open();
            }, this));*/
    ...
    $.fn.offCanvas = function(option) {
        return this.each(function() {
            var $this = $(this), data = $this.data("amui.offcanvas"), options = $.extend({}, {
                target: $this.is("a") ? $this.attr("href") : false
            }, OffCanvas.DEFAULTS, UI.utils.options($this.attr("data-am-offcanvas")), $.isPlainObject(option) && option);
            if (!data) {
